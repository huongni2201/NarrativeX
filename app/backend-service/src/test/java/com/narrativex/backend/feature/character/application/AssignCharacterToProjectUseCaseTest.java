...
class AssignCharacterToProjectUseCaseTest {
    ...
    void assignmentUsesProjectAccessAndPersistsAnAssociation() {
        Character character = Character.rehydrate(10L, 0L, "owner", null, "Mina", java.util.List.of(), CharacterStatus.ACTIVE);
        ...
        when(projectCharacterRepository.save(any(ProjectCharacter.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ...
        AssignCharacterToProjectUseCase useCase = new AssignCharacterToProjectUseCase(characterRepository,
            versionRepository, projectCharacterRepository, projectAccess, currentUserId);

        var response = useCase.execute(new AssignCharacterToProjectCommand(100L, 10L,
