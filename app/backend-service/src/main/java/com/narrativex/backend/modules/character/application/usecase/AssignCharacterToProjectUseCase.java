...
public class AssignCharacterToProjectUseCase {
    ...
    public AssignCharacterToProjectUseCase(CharacterRepository characterRepository, CharacterVersionRepository versionRepository, ProjectCharacterRepository projectCharacterRepository, ProjectAccess projectAccess, CurrentUserId currentUserId) { this.characterRepository = characterRepository; this.versionRepository = versionRepository; this.projectCharacterRepository = projectCharacterRepository; this.projectAccess = projectAccess; this.currentUserId = currentUserId; }
    @Transactional public ApiResponse<ProjectCharacter> execute(AssignCharacterToProjectCommand command) {
        ...
        ProjectCharacter assignment = ProjectCharacter.assign(command.projectId(), command.characterId(), command.role(), command.importance(), command.projectAliases(), command.storyMetadata(), command.groups(), null);
        if (command.pinnedCharacterVersionId() != null) assignment.pinVersion(versionRepository.findOwnedById(command.pinnedCharacterVersionId(), ownerId).orElseThrow(() -> new ResourceNotFoundException("Character version not found")));
