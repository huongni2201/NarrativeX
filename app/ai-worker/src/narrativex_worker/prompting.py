...
def _visual_beat_density_guidance(request: ChapterAnalysisRequest) -> str:
    ...
    return (
        ...
        "the count. Before returning JSON, count the total visual_beats across all scenes; if it is "
        "still below MIN_VISUAL_BEATS, refine again unless doing so would require inventing story "
        "events. Use shorter beats for action, reveals, reactions, or strong composition changes, "
        "and allow longer beats for genuinely stable dialogue, exposition, atmosphere, or "
        "intentionally slow moments. These are seed visual beats; the narration-aligned planner may "
        "later split or merge them using actual audio timing, and the asset resolver may reuse or "
        "reframe an image across multiple beats. Do not mechanically create a beat for every "
        "sentence, and do not invent events merely to reach the target. Long-form pacing must "
        "become progressively calmer instead of extrapolating an 8-second short-form cadence "
        "forever. "
    )
