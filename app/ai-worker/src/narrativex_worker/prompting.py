...
def build_chapter_analysis_prompt(request: ChapterAnalysisRequest) -> str:
    ...
    return (
        ...
        "the parent scene and a role of PRIMARY, SECONDARY, or BACKGROUND. PRIMARY means a visually "
        "important subject whose identity should receive a reference image first; SECONDARY is "
        "visibly participating; BACKGROUND is present but not identity-critical. Do not copy the "
        "whole scene cast into every beat. If no established character is visible in a beat, return "
        "an empty characters list. Use SOURCE_LANGUAGE as the authoritative language for the "
        "response. Every user-facing text field must be written in SOURCE_LANGUAGE, including "
        "names, aliases, descriptions, character bible and appearance text, scene titles, "
