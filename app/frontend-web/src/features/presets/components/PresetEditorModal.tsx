    ...
    <Modal isOpen={isOpen} onClose={onClose} ariaLabel={preset ? `Chỉnh sửa preset ${preset.name}` : "Tạo preset mới"} maxWidth="2xl">
      <form onSubmit={handleSubmit} className="space-y-4 p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5"><label htmlFor={categoryId} className="text-xs font-semibold text-slate-300">Danh mục</label><select id={categoryId} value={category} onChange={(event) => setCategory(event.target.value as PresetCategory)} className="w-full rounded-lg border border-slate-800 bg-[#090e18] px-3.5 py-2 text-xs text-slate-200"><option value="VISUAL_STYLE">Visual Style</option><option value="IMAGE">Image</option><option value="MOTION">Motion</option><option value="OUTFIT">Outfit</option><option value="RENDER">Render</option></select></div>
        </div>
