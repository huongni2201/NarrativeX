...
class MediaValidationRepository:
                ...
                row = await connection.fetchrow(
                    ...
                    UPDATE media_validation_jobs job
                       SET status = 'RUNNING', worker_id = $1,
                           ...
                           attempts = job.attempts + 1, updated_at = CURRENT_TIMESTAMP
                      FROM candidate
    ...
    async def complete(
        ...
        *,
        ...
        if job_update != "UPDATE 1":
            raise RuntimeError("media validation lease was lost before completion")
            ...
            async with connection.transaction():
                if job.attempts < 3:
                    result = await connection.execute(
                        """
                        UPDATE media_validation_jobs
                           SET status = 'RETRYABLE', worker_id = NULL, lease_until = NULL,
