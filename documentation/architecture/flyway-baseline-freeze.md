...
Do not edit V1-V5 after this freeze point. Every schema, index, seed, cleanup, rename, or backfill change must be added as a new versioned migration (`V6__...`, `V7__...`, and so on).
...
`FlywayBaselineFreezeTest` computes the Git blob hash of V1-V3 from the test classpath. Any accidental edit fails the backend test suite with an instruction to create a new V6+ migration instead. V4 and V5 are already published append-only refinements.
