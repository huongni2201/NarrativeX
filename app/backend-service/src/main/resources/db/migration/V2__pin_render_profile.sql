-- Pin immutable render semantics at admission time so retries cannot silently
-- change output bytes when renderer defaults evolve.
--
-- The render_input_snapshots table is part of the consolidated V1 baseline.
-- This migration only adds the profile column; it intentionally does not
-- recreate snapshot tables.

ALTER TABLE render_input_snapshots
    ADD COLUMN render_profile_json JSONB NOT NULL DEFAULT '{
      "schemaVersion": 1,
