...
public final class GenerationJobStateMachine {
  ...
  /**
   * Validates whether a transition from current to target status is permissible. Terminal states
   * (COMPLETED, FAILED, CANCELED) must never regress.
   */
    ...
    return switch (current) {
      ...
      case RUNNING ->
          target == JobStatus.RUNNING
              ...
              || target == JobStatus.UNKNOWN
              || target == JobStatus.RECONCILING;
      case UNKNOWN, RECONCILING, STALLED ->
          target == JobStatus.RUNNING
              || target == JobStatus.COMPLETED
              || target == JobStatus.FAILED
              || target == JobStatus.CANCELED
