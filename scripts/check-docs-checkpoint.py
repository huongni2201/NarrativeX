#!/usr/bin/env python3
"""Validate V1.12 documentation checkpoint semantics against Git history."""
...
SPEC = ROOT / "documentation" / "source-of-truth" / "NARRATIVEX_PROJECT_SPEC_V1_12.md"
...
PATTERNS = {
    ...
    "traceability": re.compile(
        r"implementation checkpoint `[^`]+` / `([0-9a-f]{40})`"
    ),
}
...
def main() -> int:
    ...
    if not trace_text.startswith("# NarrativeX V1.12 Baseline Implementation Traceability"):
        raise SystemExit("TRACEABILITY.md must use the V1.12 baseline traceability heading")
    ...
    if len(set(checkpoints.values())) != 1:
        raise SystemExit(
            "V1.12 baseline checkpoints disagree: "
    ...
    print(f"Documentation checkpoint semantics passed: V1.12 baseline={checkpoint}")
    return 0
