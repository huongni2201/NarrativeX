...
class DefaultMotionExecutionPolicyTest {
  ...
  @Test
  void eligibleHybridSceneBecomesI2vCandidate() {
    var resolved =
        policy.authorizeMotion(true, ProductionMode.HYBRID_LOCAL_I2V, ExecutionContext.mvp());

    assertThat(resolved.decision()).isEqualTo(MotionExecutionDecision.IMAGE_TO_VIDEO);
  }

  @Test
  void unavailableProviderFallsBackInBackendPolicy() {
    var context = new ExecutionContext(false, false, true, true, true, 0, 3);
    var resolved = policy.authorizeMotion(true, ProductionMode.HYBRID_LOCAL_I2V, context);

    assertThat(resolved.decision()).isEqualTo(MotionExecutionDecision.BASIC_IMAGE_MOTION);
    assertThat(resolved.fallbackReason()).isEqualTo(MotionFallbackReason.PROVIDER_UNAVAILABLE);
  }
}
