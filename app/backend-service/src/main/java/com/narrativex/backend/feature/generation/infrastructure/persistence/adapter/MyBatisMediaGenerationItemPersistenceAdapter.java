...
public class MyBatisMediaGenerationItemPersistenceAdapter implements MediaGenerationItemRepository {
  ...
  public MediaGenerationItem save(MediaGenerationItem item) {
    ...
    if (id == null) throw new IllegalStateException("Inserted media generation item did not return an id");
    return toDomain(mapper.findById(id));
  }

  @Override
  public Optional<MediaGenerationItem> findOwned(String userId, UUID itemId) {
    return Optional.ofNullable(mapper.findOwned(userId, itemId)).map(MyBatisMediaGenerationItemPersistenceAdapter::toDomain);
  }

  @Override
  public List<MediaGenerationItem> findByJobOwned(String userId, Long jobId) {
    return mapper.findByJobOwned(userId, jobId).stream().map(MyBatisMediaGenerationItemPersistenceAdapter::toDomain).toList();
  }
  ...
  private static MediaGenerationItemRow toRow(MediaGenerationItem item) {
    return new MediaGenerationItemRow(item.getId(), item.getRowVersion(), item.getGenerationJobId(), item.getMediaPlanId(), item.getVisualBeatId(), item.getItemKey(), item.getAttemptNumber(), item.getExecutionStatus(), item.getProviderOperationId(), item.getMediaAssetId(), item.getRequestFingerprint(), item.getErrorCode(), item.getErrorDetailRef(), item.getReviewStatus(), item.getReviewedByUserId(), item.getReviewedAt(), null, null);
  }
  ...
  private static MediaGenerationItem toDomain(MediaGenerationItemRow row) {
    ...
    return MediaGenerationItem.rehydrate(row.getId(), row.getRowVersion(), row.getGenerationJobId(), row.getMediaPlanId(), row.getVisualBeatId(), row.getItemKey(), row.getAttemptNumber(), row.getExecutionStatus(), row.getProviderOperationId(), row.getMediaAssetId(), row.getRequestFingerprint(), row.getErrorCode(), row.getErrorDetailRef(), row.getReviewStatus(), row.getReviewedByUserId(), row.getReviewedAt());
  }
}
