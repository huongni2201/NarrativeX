  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-dashed border-slate-700 bg-[#0d1420]/40 p-6">
        ...
        <h3 className="mt-2 text-base font-semibold text-slate-200">Backend API kết nối</h3>
        <p className="mt-2 text-sm leading-6 text-slate-400">
          Project và Story Version đã kết nối API thật. Các API tiếp theo cho Chapter, Scene, Visual Beats và Render đang được tiếp tục tích hợp trực tiếp từ backend.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <ProjectDatum label="Project / Story" value="Connected" />
          <ProjectDatum label="Chapter / Storyboard" value="Pending API" />
          <ProjectDatum label="Render / Export" value="Pending API" />
        </div>
      </section>
    </div>
  );
