...
public class MyBatisCharacterVersionPersistenceAdapter implements CharacterVersionRepository {
  ...
  @Override public int findMaxVersionNumberByCharacterId(Long id) { return mapper.maxCharacterVersion(id); }
  @Override public Optional<CharacterVersion> findOwnedById(Long id, String ownerId) { return Optional.ofNullable(mapper.findOwnedVersion(id, ownerId)).map(rowMapper::toDomain); }
  @Override public CharacterVersion save(CharacterVersion value) {
    CharacterVersionRow row = rowMapper.row(value, CharacterMyBatisRowMapper.InstantPair.now());
    if (value.getId() == null) { row.setId(null); row.setRowVersion(0); Long id = mapper.insertCharacterVersion(row); return rowMapper.toDomain(mapper.findCharacterVersion(id)); }
    CharacterVersionRow existing = mapper.findCharacterVersion(value.getId());
    if (existing == null) { row.setId(null); row.setRowVersion(0); Long id = mapper.insertCharacterVersion(row); return rowMapper.toDomain(mapper.findCharacterVersion(id)); }
    OptimisticConcurrency.requireVersion(value.getRowVersion(), existing.getRowVersion(), CharacterVersion.class, value.getId());
    if (mapper.updateCharacterVersion(row) != 1) throw new org.springframework.dao.OptimisticLockingFailureException("Character version was modified concurrently");
