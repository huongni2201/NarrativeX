...
- Chuyển Chapter source → detected Character identities → ProjectCharacter assignments → CharacterAppearance/Location → Scene/VisualBeat → audio → animatic → render bền vững.
...
- Analyze là explicit action theo Chapter. Production contract: `POST /api/v1/projects/{projectId}/chapters/{chapterId}/analysis-jobs`.
- Thêm/sửa Chapter chỉ reprocess affected scope; Chapter không bị ảnh hưởng được reuse.
- Trong MVP, không auto-analyze theo autosave để tránh duplicate work và chi phí ngoài ý muốn.

### Character ownership and reuse

Character is a reusable identity owned at User/Workspace scope. A Project does not own Character identity directly; it owns a ProjectCharacter assignment. The same Character may participate in multiple Projects. Visual changes such as outfit, hairstyle, aging or injuries must use CharacterAppearance/OutfitVersion instead of creating duplicate Characters. Generation always snapshots resolved CharacterVersion, CharacterAppearance, OutfitVersion and references.
...
- Chapter-first navigation và execution: add/edit/reorder/continue Chapter theo nhiều đợt; mỗi Chapter inherit project snapshot và có progress/lifecycle riêng, không regenerate phần không bị ảnh hưởng.
...
Mọi expensive operation (Chapter analyze, bulk image, TTS dài, render, AI video) phải tạo `OperationPlan` trước khi execute:

1. Resolve affected Chapter scope và snapshot duration, semantic complexity, narration/TTS, visual/motion plan, asset reuse, provider/model, quality và render profile.
...
5. Re-estimate sau Chapter Analyze, Visual Plan và provider resolution. Nếu vượt configurable threshold (baseline 20%) hoặc cap, chuyển `COST_RECONFIRMATION_REQUIRED`/`PAUSED_COST_LIMIT`.
...
- Import/analysis không yêu cầu per-story rights-attestation checkbox. Tranh chấp dùng report/review/evidence/takedown; hệ thống không suy diễn license/public domain bằng LLM.
...
Minimum Chapter analysis enqueue invariant:
...
request
  ...
  -> one transaction: OperationPlan/CostReservation + GenerationJob + StageAttempt(s) + OutboxEvent
  -> commit
  -> dispatcher
...
Until this exists, the Chapter analysis endpoint remains fail-closed and must not create fake `QUEUED` rows.
