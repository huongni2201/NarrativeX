...
import {
  ...
  MoreVertical, MousePointer2, Music2, Pause, Play, Plus, Redo2, RotateCcw, Scissors,
  Search, Settings2, SkipBack, SkipForward, SlidersHorizontal, Sparkles, Split, Trash2, Type,
...
export type ActivityId = "projects" | "assets" | "characters" | "story" | "scenes" | "voice" | "music" | "subtitles" | "transitions" | "effects" | "templates" | "settings";
...
export type DesktopScreen = ActivityId | "editor";
...
const activities: { id: ActivityId; label: string; icon: LucideIcon }[] = [
  { id: "projects", label: "Projects", icon: Folder }, { id: "assets", label: "Assets", icon: ImageIcon }, { id: "characters", label: "Characters", icon: UserCircle },
  { id: "story", label: "Story", icon: Menu }, { id: "scenes", label: "Scenes", icon: Layers3 }, { id: "voice", label: "Voice & TTS", icon: Mic2 },
  { id: "music", label: "Music", icon: Music2 }, { id: "subtitles", label: "Text / Subtitle", icon: Type }, { id: "transitions", label: "Transitions", icon: Split },
  { id: "effects", label: "Effects", icon: WandSparkles }, { id: "templates", label: "Templates", icon: Sparkles },
];

export function EditorScreen({ initialScreen = "editor" }: Readonly<{ initialScreen?: DesktopScreen }>) {
  const [activity, setActivity] = useState<ActivityId>(initialScreen === "editor" ? "projects" : initialScreen);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void workspaceApi.getRenderJob(renderJob.jobId).then(setRenderJob).catch((error) => setRenderNotice(errorMessage(error)));
    }, 2_000);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "s") { event.preventDefault(); setSaveState("Saving…"); window.setTimeout(() => setSaveState("Saved just now"), 500); }
  ...
  return <div className="desktop-app">
    <header className="topbar">
      ...
      <div className="window-actions"><button type="button" className="export-button" onClick={() => void startRender()}><Sparkles size={14} /> Export</button><button type="button" className="icon-button" aria-label="Cloud sync"><Cloud size={17} /></button><button type="button" className="icon-button notification" aria-label="Notifications"><Bell size={17} /><i /></button><button type="button" className="icon-button" aria-label="Account"><UserCircle size={22} /></button><button type="button" className="window-button" aria-label="Minimize">−</button><button type="button" className="window-button" aria-label="Maximize">□</button><button type="button" className="window-button close" aria-label="Close">×</button></div>
    </header>
    <aside className="activity-bar"><div className="activity-list">{activities.map(({ id, label, icon: Icon }) => <button key={id} type="button" className={`activity ${activity === id ? "active" : ""}`} onClick={() => { setActivity(id); setScreen(id === "projects" ? "projects" : id); }} aria-label={label} aria-pressed={activity === id} title={label}><Icon size={18} /><span>{label}</span></button>)}</div><button type="button" className="activity settings" aria-label="Settings" onClick={() => setScreen("settings")}><Settings2 size={18} /></button></aside>
    <aside className="explorer panel-right"><PanelHeader eyebrow={activity === "projects" ? "Workspace" : "Library"} title={activity === "projects" ? "Project Explorer" : activities.find((item) => item.id === activity)?.label ?? "Library"} /><>{activity === "projects" ? <ProjectTree clips={clips} selectedId={selectedId} onSelect={(id) => { setSelectedId(id); setScreen("editor"); }} projects={workspace.projects} /> : <Library activity={activity} />}</></aside>
...
function WorkspacePage({ screen, workspace, onOpenEditor }: Readonly<{ screen: ActivityId; workspace: WorkspaceState; onOpenEditor: () => void }>) {
  const titles: Record<ActivityId, string> = { projects: "Projects", assets: "Asset Browser", characters: "Character Library", story: "Story Workspace", scenes: "Scene Browser", voice: "Voice & TTS", music: "Music Library", subtitles: "Text / Subtitle", transitions: "Transitions", effects: "Effects", templates: "Templates", settings: "Desktop Settings" };
  return <div className="workspace-page"><div className="workspace-page-header"><div><span className="eyebrow">NarrativeX Desktop</span><h1>{titles[screen]}</h1><p>{screen === "projects" ? "Mở project để đi thẳng vào editor timeline." : "Nội dung được lấy từ backend domain và hiển thị trong cùng một workspace."}</p></div><div className="workspace-page-actions"><span className="connection-pill"><i /> {workspace.status === "ready" ? "Backend connected" : "API status"}</span>{screen !== "settings" && <button type="button" className="primary-action" onClick={onOpenEditor}>Open Editor <ChevronRight size={14} /></button>}</div></div><ApiStatusBanner status={workspace.status} error={workspace.error} />{screen === "projects" && <ProjectPage projects={workspace.projects} onOpenEditor={onOpenEditor} />}{screen === "assets" && <AssetsPage assets={workspace.assets} />}{screen === "characters" && <CharactersPage characters={workspace.characters} />}{screen === "story" && <StoryPage timeline={workspace.timeline} />}{screen === "scenes" && <ScenesPage timeline={workspace.timeline} />}{screen === "voice" && <VoicesPage voices={workspace.voices} />}{screen === "music" && <AssetsPage assets={workspace.assets.filter((asset) => asset.type === "AUDIO")} audioOnly />}{screen === "subtitles" && <SubtitlePage timeline={workspace.timeline} />}{screen === "transitions" && <PresetPage presets={workspace.presets.filter((preset) => preset.category === "MOTION")} title="Transition presets" />}{screen === "effects" && <PresetPage presets={workspace.presets.filter((preset) => preset.category === "MOTION")} title="Effect presets" />}{screen === "templates" && <PresetPage presets={workspace.presets} title="Style presets" />}{screen === "settings" && <SettingsPage workspace={workspace} />}</div>;
}
...
function RenderQueue({ job, notice, onStart }: Readonly<{ job: DesktopRenderJob | null; notice: string | null; onStart: () => void }>) {
  const active = Boolean(job && ["QUEUED", "RUNNING", "UNKNOWN", "STALLED", "PAUSED_COST_LIMIT"].includes(job.status));
  const statusLabel = job?.status === "COMPLETED" ? "Completed" : job?.status === "FAILED" ? "Failed" : job?.status === "CANCELED" ? "Canceled" : job?.currentStep || (active ? "Queued by backend" : "No active render job");
  return <section className="render-queue"><div className="queue-title"><div><span className="eyebrow">Render Queue</span><h3>Backend render jobs</h3></div><b>{job ? "1" : "0"}</b></div><div className="queue-body"><div className="queue-line"><span>{statusLabel}</span><strong>{job ? `${Math.max(0, Math.min(100, job.progress))}%` : "—"}</strong></div><div className="progress"><i style={{ width: `${job?.progress ?? 0}%` }} /></div>{notice && <p className="queue-notice">{notice}</p>}{job && <div className="queue-meta"><span>Job</span><b>{job.jobId.slice(0, 8)}…</b><span>Type</span><b>{job.type}</b></div>}{!job && !notice && <p className="queue-notice">Export sẽ tạo job thật qua production render API.</p>}</div><div className="queue-buttons"><button type="button" disabled title="Pause API chưa được expose trong backend"><Pause size={13} /> Pause</button><button type="button" className="queue-export" onClick={onStart} disabled={active}><Sparkles size={13} /> {active ? "Rendering" : "Export"}</button></div><div className="queue-stage"><i /> {active ? "POLLING · API CONTROLLED" : "QUEUE_IDLE · API CONTROLLED"}</div></section>;
}
