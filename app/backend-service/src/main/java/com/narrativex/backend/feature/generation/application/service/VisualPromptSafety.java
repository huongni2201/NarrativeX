...
/**
 * Adapts sensitive story wording into non-graphic still-image direction without changing plot
 * facts.
 */
public final class VisualPromptSafety {
  private static final Pattern VI_TIED = Pattern.compile("(?iu)\\b(?:bị\\s+)?trói(?:\\s+chặt)?\\b");
  ...
  private static final Pattern VI_TORTURE = Pattern.compile("(?iu)\\b(?:tra\\s+tấn|hành\\s+hạ)\\b");

  private static final Pattern EN_TIED =
      Pattern.compile("(?iu)\\b(?:tied\\s+up|bound|restrained)\\b");
  private static final Pattern EN_GAGGED = Pattern.compile("(?iu)\\b(?:gagged|gagging|gag)\\b");
  private static final Pattern EN_BLOOD = Pattern.compile("(?iu)\\b(?:bloody|blood|gore|gory)\\b");
  private static final Pattern EN_NUDE = Pattern.compile("(?iu)\\b(?:naked|nude)\\b");
  private static final Pattern EN_TORTURE = Pattern.compile("(?iu)\\b(?:torture|tortured)\\b");
