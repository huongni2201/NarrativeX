...
public class MyBatisStoryVersionPersistenceAdapter implements StoryVersionRepository {
  ...
  @Override
  public StoryVersion save(StoryVersion value) {
    return persist(value);
  }

  @Override
  public StoryVersion saveAndFlush(StoryVersion value) {
    return persist(value);
  }
  ...
  private StoryVersion persist(StoryVersion value) {
    ...
    OptimisticConcurrency.requireVersion(
        value.getRowVersion(), existing.getRowVersion(), StoryVersion.class, value.getId());
    if (mapper.update(row) != 1)
      throw new org.springframework.dao.OptimisticLockingFailureException(
          "Story version was modified concurrently");
    return toDomain(mapper.findById(value.getId()));
  }

  private static StoryVersion toDomain(StoryVersionRow row) {
