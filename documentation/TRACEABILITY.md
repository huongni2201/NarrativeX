...
| Data flow and service/module boundaries | `documentation/architecture/DATA_FLOW.md`, `SERVICE_BOUNDARIES.md` | Chapter-first durable flow documented; runtime implementation remains incomplete |
...
| Durable generation execution | `documentation/decisions/ADR-0001-system-topology-and-durable-execution.md`, `ADR-0002-chapter-first-workflow-and-routes.md` | Production contract documented; Chapter analysis enqueue is feature-gated until durable enqueue/worker execution exists |
...
|---|---|---|
| Project list/create and cursor pagination | Project controller/use cases, `CursorPage`, consolidated V1 baseline and frontend Query integration | IMPLEMENTED foundation; Create Project is metadata-only |
...
| Chapter analysis enqueue | Generation controller/use case/domain scaffold + frontend helper | **DISABLED BY DEFAULT**; route is `/api/v1/projects/{projectId}/chapters/{chapterId}/analysis-jobs`; returns `FEATURE_NOT_AVAILABLE` until durable transaction, stage/outbox dispatch, worker claim/lease and reconciliation exist |
...
8. Create Project must not enqueue AI/media work; Chapter analysis must not return `202 QUEUED` while there is no durable worker execution path.
