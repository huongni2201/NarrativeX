...
from narrativex_gpu_worker.domain.models import ArtifactAccess, ArtifactRef
from narrativex_gpu_worker.runtime.artifacts import ArtifactClient, ArtifactIntegrityError
...
async def test_download_verifies_size_and_digest() -> None:
    ...
    async with httpx.AsyncClient(transport=transport) as client:
        result = await ArtifactClient(client, 1024).download(reference(content))
...
async def test_download_rejects_digest_mismatch() -> None:
    ...
    async with httpx.AsyncClient(transport=transport) as client:
        with pytest.raises(ArtifactIntegrityError, match="integrity"):
            await ArtifactClient(client, 1024).download(reference(content, digest="0" * 64))
...
async def test_redirect_is_not_followed() -> None:
    ...
    async with httpx.AsyncClient(transport=transport) as client:
        with pytest.raises(httpx.HTTPStatusError):
            await ArtifactClient(client, 1024).download(reference(b"x"))
