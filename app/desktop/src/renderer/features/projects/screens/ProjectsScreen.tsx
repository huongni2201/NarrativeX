...
export function ProjectsScreen() {
  ...
  if (projects.isPending) {
    return (
      <main className="grid min-h-full place-items-center bg-[var(--bg)] p-10 text-sm text-[var(--text-3)]">
        Đang tải projects…
      </main>
    );
  }
  ...
  if (projects.isError) {
    return (
      <main className="grid min-h-full place-items-center bg-[var(--bg)] p-10">
        <div className="grid max-w-sm justify-items-center gap-2 text-center text-[var(--text-3)]">
          <strong className="text-[var(--text)]">Không thể tải projects</strong>
  ...
  return (
    <main className="min-h-full overflow-auto bg-[var(--bg)] p-10 md:p-14">
      <header className="mb-7 flex items-end justify-between gap-5">
        <div>
          <span className="block text-[9px] font-bold uppercase tracking-[.13em] text-[var(--text-3)]">
            Workspace
          </span>
          <h1 className="mt-1 text-[27px] font-semibold text-[var(--text)]">Projects</h1>
          <p className="mt-1 text-sm text-[var(--text-3)]">
            Tạo và mở project trực tiếp trong NarrativeX Desktop.
          </p>
        </div>
      ...
      {isCreating && (
        <form
          className="mb-6 grid max-w-xl gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-[18px]"
          onSubmit={submitProject}
        >
          <label className="grid gap-1.5 text-[11px] text-[var(--text-2)]">
            <span>Name</span>
            <Input
              autoFocus
              value={name}
          ...
          {createProject.isError && (
            <p className="m-0 text-[11px] text-[var(--danger)]" role="alert">
      ...
      {toggleFavorite.isError && (
        <p className="mb-4 text-[11px] text-[var(--danger)]" role="alert">
          Không thể cập nhật favorite: {toggleFavorite.error.message}
        </p>
      )}

      {projects.data.content.length === 0 ? (
        <div className="grid min-h-[260px] place-content-center justify-items-center gap-2 text-center text-[var(--text-3)]">
          <FolderOpen size={25} />
          <strong className="text-[var(--text)]">Chưa có project</strong>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(270px,1fr))] gap-3.5">
          {projects.data.content.map((project) => (
            <div className="relative min-w-0" key={project.id}>
              ...
              <button
                type="button"
                className="absolute right-3 top-3 text-[var(--text-3)] transition-colors hover:text-[var(--amber)] disabled:cursor-not-allowed disabled:opacity-60"
