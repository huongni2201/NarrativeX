...
export function filterVoices(voices: DesktopVoice[], filters: VoiceFilters): DesktopVoice[] {
  ...
  return voices
    .filter((voice) => {
      const searchable = `${voice.name} ${voice.language} ${voice.provider} ${voice.gender ?? ""}`.toLocaleLowerCase();
      return (
        ...
        (filters.tag === "all" || [voice.language, voice.gender, voice.provider].includes(filters.tag))
      );
    })
    .sort((left, right) => {
