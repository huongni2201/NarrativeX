    ...
    <section className="space-y-5 rounded-2xl border border-border-dark bg-surface p-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Chuẩn bị timeline trước khi ảnh hoàn tất; khi render, NarrativeX đóng băng plan revision,
          asset và các chỉnh sửa timing/motion thành một snapshot bất biến.
        </p>
      </div>
      ...
      render.status === "RESOLVING_ARTIFACT" ? (
        render.job ? (
          ...
          <p className="text-sm text-slate-300" role="status">
            Đang xác nhận render job…
          </p>
        )
      ) : null}
      {render.status === "FAILED" ? (
        <RenderFailure
          message={render.error ?? "Render thất bại."}
