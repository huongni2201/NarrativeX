...
LOGGER = logging.getLogger("narrativex.gpu_worker.runtime")
...
class TaskRuntime:
    def __init__(
        self, journal: ExecutionJournal, registry: ExecutorRegistry, max_concurrency: int
    ) -> None:
    ...
    async def _execute(self, task: ComputeTask, cancellation: asyncio.Event) -> None:
        async with self._semaphore:
            ...
            try:
                executor = self._registry.resolve(task)
