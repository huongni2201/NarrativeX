...
export const ChapterStatusBadge: React.FC<ChapterStatusBadgeProps> = ({ status, className }) => {
  ...
  if (status === "GENERATING_VISUALS") return <span className={`${base} border-badge-orange-border bg-badge-orange-bg text-badge-orange`}><Loader2 className="h-3 w-3 motion-safe:animate-spin" />Đang xử lý</span>;
