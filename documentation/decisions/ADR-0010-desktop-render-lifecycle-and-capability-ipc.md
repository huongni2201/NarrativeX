...
- Preflight receives typed executor/project/device context and passes only for `ONLINE` with valid project/device pairing, FFmpeg/ffprobe and local resources. Renderer messages are mapped from stable blocker codes.
...
- Remote materialization accepts project/asset IDs only. Main resolves metadata and a short-lived opaque capability through the backend transport, validates protocol/origin/size/type/checksum, and materializes into the controlled project workspace.
