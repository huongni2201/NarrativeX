...
export function EditorScreen({ initialScreen = "editor" }: Readonly<{ initialScreen?: DesktopScreen }>) {
  ...
  async function startRender() {
    ...
    try {
      ...
      if (!preflight.ready) {
        setRenderNotice(`Preflight blocked: ${preflight.blockers.join(" ")}`);
        return;
      }
...
function ScenesPage({ projectId, chapters, timeline }: Readonly<{ projectId: string | null; chapters: import("@narrativex/client-contracts").DesktopChapterDetails[]; timeline: DesktopTimeline | null }>) {
  ...
  async function materialize(item: import("@narrativex/client-contracts").MediaGenerationItem) {
    ...
    try { const asset = await assetsApi.get(item.mediaAssetId); const download = await assetsApi.downloadUrl(item.mediaAssetId); await window.narrativex.localStorage.materializeRemoteAsset({ projectId, assetId: asset.id, kind: asset.type, downloadUrl: download.url, sizeBytes: asset.sizeBytes, checksumSha256: asset.sha256 ?? "", filename: asset.originalFilename }); setNotice(`${asset.originalFilename} đã materialize local và checksum đã xác thực.`); } catch (error) { setNotice(errorMessage(error)); }
  }
...
function SettingsPage({ projectId, workspace }: Readonly<{ projectId: string | null; workspace: DesktopWorkspaceState }>) {
  ...
  async function backup() {
    if (!projectId) return;
    try {
      const destinationDirectory = await window.narrativex.system.selectFolder();
      if (!destinationDirectory) return;
      const result = await window.narrativex.localStorage.createBackup({ projectId, destinationDirectory });
      setNotice(`Backup đã tạo: ${result.backupDirectory}`);
    } catch (error) { setNotice(errorMessage(error)); }
  }
  async function restore() {
    try {
      const backupDirectory = await window.narrativex.system.selectFolder();
      ...
      const result = await window.narrativex.localStorage.restoreBackup({ backupDirectory, replaceExisting: true });
