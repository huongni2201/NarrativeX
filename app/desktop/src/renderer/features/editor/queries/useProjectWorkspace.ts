...
import type { DesktopAsset, DesktopCharacter, DesktopChapterDetails, DesktopPreset, DesktopProject, DesktopRenderJob, DesktopTimeline, DesktopVoice } from "@narrativex/client-contracts";
...
const emptyWorkspace: DesktopWorkspaceState = {
  status: "empty", projects: [], assets: [], characters: [], voices: [], presets: [], timeline: null, chapters: [], error: null,
};
...
export function useProjectWorkspace(projectId: string | null) {
  ...
  const chaptersQuery = useQuery({ queryKey: ["projects", projectId, "chapters", timelineQuery.data?.storyVersionId], queryFn: () => chaptersApi.list(projectId as string, timelineQuery.data?.storyVersionId as string), enabled: enabled && Boolean(timelineQuery.data?.storyVersionId) });
  ...
  const hasPending = [projectsQuery, timelineQuery, assetsQuery, charactersQuery, voicesQuery, presetsQuery, chaptersQuery].some((query) => query.isPending);
  const timeline = timelineQuery.data ?? null;
  const firstError = [projectsQuery, timelineQuery, assetsQuery, charactersQuery, voicesQuery, presetsQuery, chaptersQuery].find((query) => query.isError)?.error;
  const workspace: DesktopWorkspaceState = !projectId
    ? { ...emptyWorkspace, projects }
    : { status: hasPending ? "loading" : timeline ? "ready" : firstError ? "partial" : "empty", projects, assets: assetsQuery.data?.items ?? [], characters: charactersQuery.data?.content ?? [], voices: voicesQuery.data ?? [], presets: presetsQuery.data ?? [], timeline, chapters: chaptersQuery.data?.content ?? [], error: firstError instanceof Error ? firstError.message : null };
