"""Production materialization behavior for the NarrativeX worker."""
...
class ProductionWorkerRepository(WorkerRepository):
    """Production repository with revision-safe storyboard materialization."""
    ...
    async def _materialize_storyboard(
        ...
        # Replays may replace only the job's own draft revision. Historical/current revisions are
        # never touched, so approved output remains durable until the new revision commits.
        ...
        # Visibility switches only after the target revision is fully materialized. This update is
        # part of the same transaction as the writes above, so any failure rolls everything back.
