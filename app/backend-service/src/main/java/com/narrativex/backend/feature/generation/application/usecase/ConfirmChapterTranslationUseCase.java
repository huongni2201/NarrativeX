...
public class ConfirmChapterTranslationUseCase {
  ...
  public GenerationJob execute(
      ConfirmChapterTranslationCommand command) {
    ...
    chapter = chapterRepository.findById(chapterId).orElseThrow(() -> new ResourceNotFoundException("Chapter not found"));
    storyVersionAccess.requireOwnedStoryVersion(projectId, chapter.getStoryVersionId(), userId);
    ...
    BigDecimal maxAuthorized = BigDecimal.valueOf(Math.min(0.25d, 0.02d + estimatedTokens / 250_000d)).setScale(6, RoundingMode.UP);
    var reservation = quotaReservation.reserve(userId, maxAuthorized, quota.maxConcurrentExpensiveJobs())
        .orElseThrow(() -> new GenerationAdmissionDeniedException("COST_LIMIT", "The translation quota is exhausted."));
    OperationPlan plan = operationPlanRepository.save(OperationPlan.create(
