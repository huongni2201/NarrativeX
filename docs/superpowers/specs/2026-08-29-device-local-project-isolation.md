...
Creating a project still creates the backend project state required by analysis/generation, then immediately registers that project in the local catalog. A project ID absent from the local catalog is not opened by the Desktop project-detail flow, and project-scoped workspace queries must not run until that local registration check succeeds.
...
- project-scoped workspace queries remain disabled until local project validation succeeds;
