...
export function EditorScreen({ initialScreen = "editor" }: Readonly<{ initialScreen?: DesktopScreen }>) {
  ...
  return <div className={`desktop-app ${screen === "voice" ? "voice-library-app" : ""}`}>
    <header className="topbar">
      <div className="brand"><img src="/branding/narrativex-icon.png" alt="NarrativeX" width={22} height={22} className="brand-logo-img" /><span>NarrativeX</span></div>
    ...
    <aside className="explorer panel-right"><PanelHeader eyebrow={activity === "voice" ? "Library / Voice & TTS" : activity === "editor" || activity === "chapters" ? "Workspace" : "Library"} title={activity === "editor" || activity === "chapters" ? "Project Explorer" : activities.find((item) => item.id === activity)?.label ?? "Library"} /><>{activity === "editor" || activity === "chapters" ? <ProjectTree clips={clips} selectedId={selectedId} onSelect={(id) => { setSelectedId(id); setActivity("editor"); setScreen("editor"); }} projects={workspace.projects} activeProjectId={selectedProjectId} /> : <Library activity={activity} />}</></aside>
    <main className="center-workspace">
      {screen === "editor" ? <div className="editor-surface">
      <ApiStatusBanner status={workspace.status} error={workspace.error} />
      <section className="preview"><PanelHeader eyebrow="Canvas" title="Preview" trailing={<><span className="preview-mode"><i /> FAST PREVIEW</span><Select defaultValue="1080p"><SelectTrigger className="select-control" aria-label="Preview resolution"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="720p">720p</SelectItem><SelectItem value="1080p">1080p</SelectItem><SelectItem value="4k">4K</SelectItem></SelectContent></Select><IconButton label="Snapshot"><Camera size={16} /></IconButton><IconButton label="More options"><MoreVertical size={16} /></IconButton></>} /><div className="preview-stage"><div className="preview-art" /><div className="preview-overlay" /><div className="preview-caption">{selected ? selected.title : "Chọn một VisualBeat để bắt đầu"}</div><span className="preview-label"><i /> PREVIEW / {selected?.scene ?? "No scene selected"}</span></div><div className="transport"><span className="current-time">{formatTimecode(playheadMs)}</span><span className="slash">/</span><span>{totalMs ? formatTimecode(totalMs) : "--:--:--:--"}</span><div className="transport-buttons"><IconButton label="Previous clip" onClick={() => selected && setPlayheadMs(Math.max(0, selected.startMs - 1000))}><SkipBack size={15} /></IconButton><IconButton label="Step backward" onClick={() => setPlayheadMs((value) => Math.max(0, value - 500))}><SkipBack size={18} /></IconButton><button type="button" className="play-button" aria-label={playing ? "Pause" : "Play"} onClick={() => setPlaying((value) => !value)} disabled={!totalMs}>{playing ? <Pause size={18} /> : <Play size={18} fill="currentColor" />}</button><IconButton label="Step forward" onClick={() => setPlayheadMs((value) => Math.min(totalMs, value + 500))}><SkipForward size={18} /></IconButton><IconButton label="Next clip" onClick={() => selected && setPlayheadMs(Math.min(totalMs, selected.endMs + 1000))}><SkipForward size={15} /></IconButton><IconButton label="Loop"><RotateCcw size={16} /></IconButton></div><div className="transport-extra"><IconButton label="Fullscreen"><Maximize2 size={16} /></IconButton><input aria-label="Preview volume" type="range" min="0" max="100" defaultValue="72" /><Volume2 size={16} /></div></div></section>
...
function IconButton({ label, children, onClick, active = false, disabled = false, side = "bottom" }: Readonly<{ label: string; children: ReactNode; onClick?: () => void; active?: boolean; disabled?: boolean; side?: "top" | "right" | "bottom" | "left" }>) { return <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className={`icon-button ${active ? "active" : ""}`} aria-label={label} title={label} onClick={onClick} disabled={disabled}>{children}</Button></TooltipTrigger><TooltipContent side={side}>{label}</TooltipContent></Tooltip>; }
...
function VoicesPage({ projectId, chapters, voices, assets }: Readonly<{ projectId: string | null; chapters: import("@narrativex/client-contracts").DesktopChapterDetails[]; voices: DesktopVoice[]; assets: DesktopAsset[] }>) {
  ...
  return <div className="voice-library">
    <div className="voice-library-header">
      <div><span className="eyebrow">Library / Voice &amp; TTS</span><h1>Voice &amp; TTS</h1><p>Quản lý và phát thử các file audio mẫu (TTS) trong workspace của bạn.</p></div>
      <div className="voice-header-actions"><span className="voice-file-count">{voices.length} files</span><button type="button" className="primary-action" onClick={() => void importReference()}><Upload size={14} /> Upload audio</button><button type="button" className="icon-button voice-more" aria-label="More voice library options" title="More options"><MoreVertical size={17} /></button></div>
    </div>
    <div className="voice-filter-bar">
      <label className="voice-search"><Search size={15} /><span className="sr-only">Tìm kiếm voice</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm kiếm tên, giọng đọc, ngôn ngữ..." /></label>
      <label className="voice-filter"><span>Ngôn ngữ</span><select value={language} onChange={(event) => setLanguage(event.target.value)}><option value="all">Tất cả</option>{languages.map((value) => <option value={value} key={value}>{value}</option>)}</select></label>
      <label className="voice-filter"><span>Giới tính</span><select value={gender} onChange={(event) => setGender(event.target.value)}><option value="all">Tất cả</option>{genders.map((value) => <option value={value} key={value}>{formatVoiceGender(value)}</option>)}</select></label>
      <label className="voice-filter"><span>Nhà cung cấp</span><select value={provider} onChange={(event) => setProvider(event.target.value)}><option value="all">Tất cả</option>{providers.map((value) => <option value={value} key={value}>{value}</option>)}</select></label>
      <label className="voice-filter voice-sort"><span>Sắp xếp</span><select value={sortOrder} onChange={(event) => setSortOrder(event.target.value)}><option value="newest">Mới nhất</option><option value="name">Tên A–Z</option></select></label>
      <div className="voice-view-toggle" role="group" aria-label="Kiểu hiển thị"><button type="button" className={viewMode === "grid" ? "active" : ""} aria-label="Grid view" aria-pressed={viewMode === "grid"} onClick={() => setViewMode("grid")}><Layers3 size={16} /></button><button type="button" className={viewMode === "list" ? "active" : ""} aria-label="List view" aria-pressed={viewMode === "list"} onClick={() => setViewMode("list")}><Menu size={16} /></button></div>
    </div>
    <details className="voice-generation-panel">
      <summary><span><Mic2 size={14} /> Tạo narration từ voice catalog</span><ChevronDown size={15} /></summary>
      <div className="generation-toolbar voice-generation-controls"><label>Mode<select value={audioMode} onChange={(event) => setAudioMode(event.target.value as typeof audioMode)}><option value="TTS">Narration / TTS</option><option value="USER_AUDIO">Use my audio</option></select></label><label>Voice<select value={voiceId} onChange={(event) => setVoiceId(event.target.value)} disabled={audioMode === "USER_AUDIO"}><option value="">Chọn voice</option>{voices.map((voice) => <option value={voice.id} key={voice.id}>{voice.name} · {voice.language}</option>)}</select></label><label>Chapter<select value={chapterId} onChange={(event) => setChapterId(event.target.value)}><option value="">Chọn chapter</option>{chapters.map((chapter) => <option value={chapter.id} key={chapter.id}>{chapter.orderIndex + 1}. {chapter.title}</option>)}</select></label><label>Local audio<select value={referenceAssetId} onChange={(event) => setReferenceAssetId(event.target.value)}><option value="">None</option>{audioAssets.map((asset) => <option value={asset.id} key={asset.id}>{asset.originalFilename}</option>)}</select></label><button type="button" className="outline-action" onClick={() => void importReference()}><Plus size={13} /> Import user audio</button><button type="button" className="primary-action" onClick={() => void generateChapter()} disabled={!projectId || !chapterId || !voiceId || generate.isPending || audioMode === "USER_AUDIO"}><Mic2 size={13} /> Generate chapter</button><button type="button" className="outline-action" onClick={() => void generateAll()} disabled={!projectId || !voiceId || chapters.length === 0 || generateBatch.isPending || audioMode === "USER_AUDIO"}>Generate all</button></div>
    </details>
    {notice && <p className="queue-notice voice-notice" role="status">{notice}</p>}{job.data && <div className="generation-job voice-job"><span className="eyebrow">Narration job</span><strong>{job.data.type} · {job.data.status} · {job.data.progress}%</strong><small>{job.data.currentStep ?? "Waiting for worker"}</small></div>}
    <div className="voice-results-line"><span>{filteredVoices.length} voice{filteredVoices.length === 1 ? "" : "s"}</span><span>{voices.length === filteredVoices.length ? "Voice catalog" : `${voices.length - filteredVoices.length} đang được ẩn bởi bộ lọc`}</span></div>
    <div className={`voice-grid ${viewMode === "list" ? "is-list" : ""}`}>{filteredVoices.map((voice) => <VoiceAudioCard voice={voice} key={voice.id} />)}{filteredVoices.length === 0 && <ResourceEmpty title={voices.length === 0 ? "Chưa tải voice catalog" : "Không tìm thấy voice phù hợp"} action={voices.length === 0 ? "Kiểm tra Catalog API" : "Thử xoá bớt bộ lọc hoặc từ khoá tìm kiếm"} />}</div>
  </div>;
}
...
function VoiceAudioCard({ voice }: Readonly<{ voice: DesktopVoice }>) {
  ...
  return <article className="voice-audio-card"><div className="voice-art"><button type="button" className="voice-play-button" aria-label={`${playing ? "Dừng" : "Phát thử"} ${voice.name}`} onClick={() => void togglePlayback()} disabled={!voice.sampleUrl}>{playing ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}</button></div><div className="voice-card-main"><div className="voice-card-heading"><div><h3>{voice.name}</h3><p>{formatVoiceGender(voice.gender ?? "Voice preset")} · {voice.language}</p></div><button type="button" className="voice-card-more" aria-label={`Tuỳ chọn cho ${voice.name}`} title="More options"><MoreVertical size={15} /></button></div><div className="voice-card-footer"><button type="button" className="voice-mini-play" aria-label={`${playing ? "Dừng" : "Phát thử"} ${voice.name}`} onClick={() => void togglePlayback()} disabled={!voice.sampleUrl}>{playing ? <Pause size={12} fill="currentColor" /> : <Play size={12} fill="currentColor" />}</button><div className="voice-waveform" aria-hidden="true">{[5, 12, 18, 8, 24, 11, 17, 7, 22, 14, 9, 19, 12, 25, 8, 16, 10, 21, 13, 7, 18, 11, 22, 9, 15, 6, 19, 12, 8, 16].map((height, index) => <i key={`${voice.id}-wave-${index}`} style={{ height: `${height}px` }} />)}</div><span className="voice-duration">{voice.sampleUrl ? "Preview" : "No audio"}</span></div></div>{voice.sampleUrl && <audio ref={audioRef} preload="none" src={voice.sampleUrl} onEnded={() => setPlaying(false)} />}</article>;
}
...
function VoiceContextPanel({ workspace }: Readonly<{ workspace: DesktopWorkspaceState }>) {
  ...
  return <div className="voice-context-panel"><div className="voice-context-heading"><span className="eyebrow">Workspace context</span><h3>Voice</h3></div><div className="voice-context-metrics"><div><span>Danh sách tệp đã chọn</span><strong>0 file</strong></div><div><span>Tổng thời lượng</span><strong>00:00</strong></div><div><span>Tổng dung lượng</span><strong>{formatBytes(audioBytes)}</strong></div></div><div className="voice-dropzone"><Upload size={20} /><strong>Kéo &amp; thả file audio vào đây</strong><span>hoặc nhấn để chọn</span><small>MP3, WAV, M4A (Tối đa 50MB / file)</small></div><button type="button" className="voice-folder-action"><Plus size={14} /> Tạo thư mục</button><div className="voice-context-section"><span className="context-section-title">Thẻ phổ biến</span>{tags.length ? tags.map((tag) => <div className="voice-tag-row" key={tag.label}><span>{tag.label}</span><strong>{tag.value}</strong></div>) : <p className="context-empty">Chưa có voice tag.</p>}<button type="button" className="voice-more-link">Xem thêm <ChevronDown size={13} /></button></div><div className="voice-quick-actions"><span className="eyebrow">Quick actions</span><button type="button"><Mic2 size={14} /><span>Voice catalog</span><ChevronRight size={14} /></button><button type="button"><Plus size={14} /><span>Tạo voice take</span><ChevronRight size={14} /></button></div></div>;
}
