...
public final class ProjectRenderProfileFactory {
  ...
  public static String create(int fps, boolean subtitlesEnabled) {
    if (fps != 30 && fps != 60) {
      throw new IllegalArgumentException("fps must be 30 or 60");
    }
    String subtitleMode = subtitlesEnabled ? "burn_in" : "none";
    return """
        {"schemaVersion":2,"rendererVersion":"%s","compositionPolicyVersion":%d,"fps":%d,"video":{"x264Preset":"medium","crf":18,"nvencPreset":"p6","nvencCq":19,"pixelFormat":"yuv420p"},"color":{"mode":"SDR_BT709_LIMITED"},"subtitles":{"mode":"%s"}}
        """
        .formatted(RENDERER_VERSION, COMPOSITION_POLICY_VERSION, fps, subtitleMode)
        .strip();
  }
}
