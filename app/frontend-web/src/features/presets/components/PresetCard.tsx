...
export const PresetCard: React.FC<PresetCardProps> = ({ preset, isSelected = false, onClick }) => (
  <article className={cn("group overflow-hidden rounded-2xl border bg-surface shadow-md transition-colors", isSelected ? "border-orange-500 ring-2 ring-orange-500/50" : "border-slate-800/90 hover:border-slate-700")}>
    <button type="button" onClick={onClick} className="block w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-950">
        ...
        />
        {isSelected && <span className="absolute right-2.5 top-2.5 flex h-6 w-6 items-center justify-center rounded-full border border-orange-400 bg-orange-600 text-white"><Check className="h-3.5 w-3.5" /></span>}
        {preset.isDefault && <span className="absolute left-2.5 top-2.5 rounded-md border border-orange-700/60 bg-orange-950/90 px-2 py-0.5 text-[10px] font-semibold text-orange-300">Mặc định</span>}
      </div>
      <div className="space-y-3 p-4"><div><h3 className="text-sm font-bold text-white">{preset.name}</h3><p className="mt-1 line-clamp-3 text-xs leading-relaxed text-slate-400">{preset.description}</p></div><div className="flex flex-wrap gap-1.5">{preset.tags.map((tag) => <span key={tag} className="rounded border border-slate-800 bg-surface-panel px-2 py-0.5 font-mono text-[10px] font-bold text-slate-400">{tag}</span>)}</div></div>
    </button>
