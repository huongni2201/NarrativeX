...
Gemini Web generation applies a Desktop-main-owned series style lock before the Visual Beat scene prompt is submitted. The current lock targets a premium cinematic emotional anime rendering language: delicate Japanese anime aesthetics, expressive glossy eyes, flowing wind-swept hair, soft painterly cel shading, luminous backlight and rim light, dreamy atmospheric bloom, shallow depth of field, soft bokeh, foreground petals/flowers when appropriate, and a poetic wistful visual tone with consistent serialized illustration quality. The style lock is intentionally separate from scene content so it does not force a specific character design, hair color, historical era, costume, or location that the canonical Scene/Character context does not request.
