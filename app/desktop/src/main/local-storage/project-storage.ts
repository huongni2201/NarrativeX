...
export class ProjectStorage {
  ...
  async registerAsset(
    ...
    const relativePath = join("assets", assetDirectory(input.kind), `${input.assetId}${extension}`);
    ...
    const checksumSha256 = await verifyOrCalculateChecksum(destination, input.checksumSha256);
  ...
  async registerArtifact(
    ...
    const relativePath = join("artifacts", input.jobId, `final${safeExtension(source) || ".mp4"}`);
    const destination = this.resolveProjectRelativePath(projectId, relativePath);
    await mkdir(dirname(destination), { recursive: true });
    if (source !== destination) await copyFile(source, destination);
  ...
  async resolveAsset(
    ...
    if (!entry) throw new Error(`Local asset ${assetId} is not registered for project ${projectId}.`);
    ...
    const expectedChecksum = normalizeChecksum(expected?.checksumSha256 ?? entry.checksumSha256);
    if (expectedChecksum) {
  ...
  private projectRoot(projectId: string): string {
    if (!PROJECT_ID_PATTERN.test(projectId)) throw new Error("Invalid projectId for local storage.");
  ...
  private resolveProjectRelativePath(projectId: string, relativePath: string): string {
    if (!relativePath || isAbsolute(relativePath)) throw new Error("Local project path must be relative.");
    const root = this.projectRoot(projectId);
    const candidate = resolve(root, normalize(relativePath));
    const rel = relative(root, candidate);
    if (!rel || rel === ".") return candidate;
    if (rel.startsWith("..") || isAbsolute(rel)) throw new Error("Local project path escapes its workspace.");
    return candidate;
  }
  ...
  private async writeManifest(manifest: LocalProjectManifest): Promise<void> {
    ...
    const temporary = join(root, `.project.manifest.${process.pid}.${Date.now()}.tmp`);
...
function normalizeChecksum(value: string | undefined): string | null {
  ...
  if (!SHA256_PATTERN.test(normalized)) throw new Error("Checksum must be lowercase SHA-256 hex.");
  return normalized;
}

async function verifyOrCalculateChecksum(path: string, expected?: string): Promise<string> {
