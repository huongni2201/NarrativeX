...
- `media_assets.storage_mode` and device-scoped `local_media_materializations` are part of the clean V1 baseline. The registry stores asset/device availability and checksum metadata only; absolute local paths remain exclusively in Desktop project manifests.
