...
VISUAL_DIRECTION_INSTRUCTIONS = (
    ...
    "Preserve continuity with the canonical character identity, current appearance, location canon, "
    "and time-of-day while describing only what changes in this beat. Describe: (1) the primary "
    "subject and one readable physical action or state, (2) environment state and the story-relevant "
    "prop/detail, (3) composition and what must remain visually dominant, (4) one physically "
    "motivated light source and atmosphere, and (5) restrained facial/body performance when a "
    "character is present. Do not re-specify permanent facial features, body proportions, hair "
    "color, stable hair silhouette, skin tone, or other identity traits inside visual_intent; those "
    "come from the canonical character visual_prompt. Only describe transient changes required by "
    "this beat, such as pose, expression, current action, temporary damage, or an explicitly changed "
    "wardrobe state. Do not duplicate the persistent location visual canon; describe only "
    "scene-relevant environmental state or changes. Also set camera_angle to exactly one structured "
    "value from the response schema: WIDE, MEDIUM, CLOSE_UP, EXTREME_CLOSE_UP, LOW_ANGLE, "
    "HIGH_ANGLE, OVER_THE_SHOULDER, or POV. Choose the framing that best communicates the beat; do "
    "not use camera_angle for camera movement. Prefer concrete visible details over adjectives such "
    "as cinematic, beautiful, epic or dramatic. Do not invent text, logos, subtitles, watermarks, "
