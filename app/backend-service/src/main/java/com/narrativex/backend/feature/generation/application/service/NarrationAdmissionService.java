...
public class NarrationAdmissionService {
  ...
  public void admit() {
    if (!limits.isNarrationEnabled()) {
      throw new FeatureNotAvailableException("Narration is not enabled.");
    }
    ...
    if (generationJobRepository.countActiveJobs() >= limits.getMaxConcurrentExpensiveJobs()) {
      throw new GenerationAdmissionDeniedException(
          "CAPACITY_LIMIT", "The narration concurrency limit is exhausted.");
    }
  }
}
