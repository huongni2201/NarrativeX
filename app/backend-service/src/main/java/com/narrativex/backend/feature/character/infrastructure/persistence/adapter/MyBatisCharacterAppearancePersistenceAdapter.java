...
public class MyBatisCharacterAppearancePersistenceAdapter implements CharacterAppearanceRepository {
  ...
  @Override public CharacterAppearance save(CharacterAppearance value) {
    CharacterAppearanceRow row = rowMapper.row(value, CharacterMyBatisRowMapper.InstantPair.now());
    if (value.getId() == null) { row.setId(null); row.setRowVersion(0); Long id = mapper.insertAppearance(row); return rowMapper.toDomain(mapper.findAppearance(id)); }
    CharacterAppearanceRow existing = mapper.findAppearance(value.getId());
    if (existing == null) { row.setId(null); row.setRowVersion(0); Long id = mapper.insertAppearance(row); return rowMapper.toDomain(mapper.findAppearance(id)); }
    OptimisticConcurrency.requireVersion(value.getRowVersion(), existing.getRowVersion(), CharacterAppearance.class, value.getId());
    if (mapper.updateAppearance(row) != 1) throw new org.springframework.dao.OptimisticLockingFailureException("Character appearance was modified concurrently");
