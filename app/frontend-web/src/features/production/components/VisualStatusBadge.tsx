...
export const VisualStatusBadge: React.FC<VisualStatusBadgeProps> = ({ status, className, size = "md" }) => {
  ...
  if (status === "PROCESSING") return <span className={cn(base, spacing, "text-badge-blue bg-badge-blue-bg border border-badge-blue-border", className)}><Loader2 className={cn("animate-spin", isSm ? "w-3 h-3" : "w-3.5 h-3.5")} />PROCESSING</span>;
