...
6. Write provider output to worker-local scratch, validate MIME, checksum, positive dimensions, requested ratio/crop strategy and provider response schema, upload the immutable validated object to Cloudflare R2, persist Asset/MediaAsset metadata in PostgreSQL, then complete the stage. Local scratch may be deleted after durable persistence succeeds.
