...
export const ChapterWorkspace: React.FC = () => {
  const {
    ...
    useShallow((state) => ({
      ...
      setActiveWorkspaceTab: state.setActiveWorkspaceTab,
      visualBeats: state.visualBeats,
  ...
  const chapter =
    ...
    project.chapters.find((candidate) => candidate.number === "01") ||
    project.chapters[0];

  if (!chapter) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-700 bg-[#0d1420]/50 p-8 text-sm text-slate-400">
        Project này chưa có chapter để mở workspace.
      </div>
    );
  }
  ...
  const currentActiveScene = scenes.find((scene) => scene.id === activeSceneId) || scenes[0];
      ...
      <div className="p-5 md:p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-xl">
        ...
        <div className="flex flex-wrap items-center gap-6 w-full lg:w-auto justify-between lg:justify-end border-t lg:border-t-0 border-slate-800 pt-4 lg:pt-0">
          <div className="flex items-center gap-6 text-center">
            <div>
              <div className="text-xl font-bold text-white font-mono">{chapter.scenesCount ?? scenes.length}</div>
              <div className="text-[11px] text-slate-400">Scenes</div>
            </div>
            <div>
              <div className="text-xl font-bold text-white font-mono">{chapter.totalVisualsCount ?? visualBeats.length}</div>
            ...
            <div>
              <div className="text-xl font-bold text-white font-mono flex items-center justify-center gap-1">
                ...
                <span>{chapter.duration || "00:00"}</span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 flex-1">
              <div className="relative">
                <select
                  ...
                  onChange={(event) => handleSceneSelect(event.target.value)}
                ...
                >
                  {scenes.map((scene) => (
                    <option key={scene.id} value={scene.id} className="bg-[#0d1420] text-slate-200">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            <div className="w-full lg:w-72 xl:w-80 p-4 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-3 shrink-0 shadow-lg">
              ...
              <div className="space-y-2 pt-1">
                {scenes.map((scene) => {
                  const isActive = scene.id === effectiveSceneId;
                  return (
                    <button
                      key={scene.id}
                      type="button"
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-white flex items-center gap-1.5">
                          <span className="text-purple-400 font-mono">{scene.number}</span>
                          <span>{scene.title}</span>
                        </span>
                        <span className="text-[11px] font-mono text-slate-400">{scene.beatsCount ?? 0} beats</span>
                      </div>
            ...
            <div className="flex-1 w-full space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">
                  SCENE {currentActiveScene.number} – {currentActiveScene.title.toUpperCase()} ({filteredBeats.length} BEATS)
                </h3>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredBeats.map((beat) => (
                  <button
                    ...
                    key={beat.id}
                    onClick={() => setView("visual-review")}
                    className="group text-left bg-[#0d1420] hover:bg-[#111a29] border border-slate-800 hover:border-purple-500/60 rounded-xl overflow-hidden cursor-pointer transition-all duration-200 shadow-md flex flex-col justify-between focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500"
                  >
                    ...
                    <div className="p-3 space-y-2.5">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
                        ...
                        {beat.status !== "APPROVED" && beat.status !== "NEEDS_REVIEW" && beat.status !== "REJECTED" && (
                          <VisualStatusBadge status={beat.status} size="sm" />
                        )}

                        <span className="text-[10px] text-slate-400 font-mono">{beat.durationSeconds}s</span>
                      </div>
                    </div>
                  </button>
                ))}
                ...
                <button
                  type="button"
                  onClick={() => setView("visual-review")}
                  className="border-2 border-dashed border-slate-800 hover:border-purple-500/60 bg-[#090e18]/40 hover:bg-purple-950/20 rounded-xl flex flex-col items-center justify-center p-6 text-center space-y-2.5 cursor-pointer transition-all min-h-[190px] group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500"
                >
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-5 p-5 md:p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-6">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-200">Tiến trình Chapter</h2>
            ...
            <div className="space-y-4">
              <div className="p-3 rounded-xl bg-[#090e18] border border-slate-800/80 flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-200">Phân tích Chapter</span>
                    <span className="text-[10px] text-emerald-400 font-mono font-medium">Hoàn thành: 17/08/2026 14:20</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-[#090e18] border border-slate-800/80 flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-slate-200">Lập kế hoạch Visual Beats</span>
                    <span className="text-[10px] text-emerald-400 font-mono font-medium">Hoàn thành: 17/08/2026 14:45</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-purple-950/40 border border-purple-800/60 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">Generate Visuals</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-purple-300">Đang xử lý 32/38 (84%)</span>
                </div>
              ...
              <div className="p-3 rounded-xl bg-[#090e18] border border-slate-800/80 flex items-center justify-between opacity-70">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-medium text-slate-300">Audio (TTS &amp; Subtitle)</span>
                </div>
              ...
              <div className="p-3 rounded-xl bg-[#090e18] border border-slate-800/80 flex items-center justify-between opacity-70">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-medium text-slate-300">Render Chapter</span>
                </div>
                <span className="text-[11px] text-slate-500 font-mono">Chờ xử lý</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="p-5 md:p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-white">Scenes ({scenes.length})</h3>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
                {scenes.slice(0, 4).map((scene) => (
                  <button
                    ...
                    className="group relative text-left bg-[#090e18] hover:bg-[#111a29] border border-slate-800 hover:border-purple-500/60 rounded-xl overflow-hidden cursor-pointer transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500"
                  >
      ...
      {activeTab === "content" && (
        <div className="p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-mono">{scenes.length} scenes</span>
          </div>
          <textarea
            defaultValue={
              chapter.storyExcerpt ||
          ...
          <div className="flex justify-end gap-3">
            <Button variant="outline" size="sm" onClick={() => setActiveWorkspaceTab("storyboard")}>
              Hủy
            </Button>
            <Button variant="primary" size="sm" onClick={() => setActiveWorkspaceTab("storyboard")}>
              Lưu thay đổi
            </Button>
          </div>
        </div>
      ...
      {activeTab === "visuals" && (
        <div className="p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="primary" size="sm" onClick={() => setView("visual-review")}>
      ...
      {activeTab === "render" && (
        <div className="p-6 rounded-2xl bg-[#0d1420] border border-slate-800/90 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="gradient" size="sm" onClick={() => setView("render")}>
              Render ngay
            </Button>
          </div>
          <p className="text-sm text-slate-400">
