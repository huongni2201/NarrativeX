...
export interface EffectiveDesktopPreferences {
  userId: string;
  gemini: GeminiTabCounts & { environmentDefaults: GeminiTabCounts };
  window: SavedWindowState | null;
}

type StoredProfile = {
  gemini?: Partial<GeminiTabCounts>;
...
function sanitizeProfile(value: unknown): StoredProfile {
  ...
  if (profile.gemini && typeof profile.gemini === "object") {
    ...
    const next: Partial<GeminiTabCounts> = {};
    ...
    if (Object.keys(next).length) sanitized.gemini = next;
  }
...
function sanitizePreferences(value: unknown): StoredPreferences {
  ...
  if (input.schemaVersion !== 1 || !input.profiles || typeof input.profiles !== "object") {
...
export class DesktopPreferencesStore {
  ...
  async getLastActive(): Promise<EffectiveDesktopPreferences | null> {
    ...
    return this.effective(
      state.lastActiveUserId,
      state.profiles[state.lastActiveUserId] ?? {},
    );
  }
