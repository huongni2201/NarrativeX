...
export const PresetDetailDrawer: React.FC<PresetDetailDrawerProps> = ({ preset, onClose, onEdit, onDuplicate, onDelete }) => {
  ...
  return (
    <aside aria-label={`Chi tiết preset ${preset.name}`} className="flex h-full w-full shrink-0 flex-col overflow-y-auto border-l border-slate-800 bg-surface shadow-2xl lg:w-[380px] xl:w-[420px]">
      ...
      <div className="flex-1 space-y-5 p-5 text-xs">
        <p className="rounded-xl border border-slate-800 bg-surface-panel p-3.5 leading-relaxed text-slate-300">{preset.description}</p>
        <div><h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Màu sắc chủ đạo</h4><div className="mt-2 flex gap-2">{(preset.colorPalette ?? ["var(--background)", "var(--primary)"]).map((color) => <span key={color} title={color} className="h-7 w-7 rounded-lg border border-white/20" style={{ backgroundColor: color }} />)}</div></div>
        <dl className="space-y-2 rounded-xl border border-slate-800 bg-surface-panel p-3.5"><div className="flex justify-between gap-3"><dt className="text-slate-500">Ánh sáng</dt><dd className="text-right text-slate-200">{preset.lighting ?? "—"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Camera</dt><dd className="text-right text-slate-200">{preset.cameraStyle ?? "—"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Tỷ lệ</dt><dd className="font-mono text-orange-300">{preset.defaultAspectRatio ?? "16:9"}</dd></div></dl>
      </div>
