...
public class MyBatisStoryboardPersistenceAdapter implements StoryboardRepository {
  ...
  @Override
  public VisualBeat saveVisualBeat(VisualBeat value) {
    ...
    row.setSceneId(value.getSceneId());
    ...
    if (existing == null) throw new ResourceNotFoundException("Visual beat not found");
    OptimisticConcurrency.requireVersion(
        value.getRowVersion(), existing.getRowVersion(), VisualBeat.class, value.getId());
    if (mapper.updateVisualBeat(row) != 1)
      throw new org.springframework.dao.OptimisticLockingFailureException(
          "Visual beat was modified concurrently");
    return toDomain(mapper.findVisualBeat(value.getId()));
  }

  private static Scene toDomain(SceneRow row) {
    return Scene.rehydrate(
        ...
        SceneStatus.valueOf(row.getStatus()));
  }

  private static VisualBeat toDomain(VisualBeatRow row) {
    VisualBeat beat =
