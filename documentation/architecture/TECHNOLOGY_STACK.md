...
| Persistence | PostgreSQL driver, Flyway, Spring Data JPA; two-file baseline `V1__initial_schema.sql` plus deterministic `V2__seed_demo_data.sql` | Authoritative transactional domain/job/cost/safety state |
...
| Migrations | Flyway two-file development baseline: consolidated V1 schema plus V2 demo seed | PostgreSQL bootstrap; after release, schema changes must use forward-only migrations |
