  ...
  return (
    ...
    >
      <div className="relative flex aspect-[16/10] w-full items-center justify-center overflow-hidden bg-gradient-to-br from-[#171434] via-[#10172b] to-[#09111d]">
        ...
        <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded bg-black/40 px-2 py-1 text-[10px] font-medium uppercase tracking-wider text-purple-200">
          <Server className="h-3 w-3" /> API
        </div>
      </div>

      <div className="flex flex-1 flex-col justify-between gap-4 p-3.5">
        <div>
          <h3 className="truncate text-sm font-semibold text-slate-100 transition-colors group-hover:text-purple-300">
            {project.name}
          </h3>
          <p className="mt-1 text-xs text-slate-500">Project #{project.id} · row {project.rowVersion}</p>
        </div>
