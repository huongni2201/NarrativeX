...
CREATE INDEX idx_projects_active_owner_updated_id
    ON projects (owner_id, updated_at DESC, id DESC) WHERE archived_at IS NULL;
CREATE INDEX idx_projects_owner_status_updated_active
    ON projects (owner_id, status, updated_at DESC, id DESC) WHERE archived_at IS NULL;
CREATE INDEX idx_projects_owner_created_active
    ON projects (owner_id, created_at ASC, id ASC) WHERE archived_at IS NULL;
CREATE INDEX idx_projects_owner_lower_name_active
    ON projects (owner_id, LOWER(name), id) WHERE archived_at IS NULL;
CREATE INDEX idx_project_favorites_project_user ON project_favorites (project_id, user_id);
...
CREATE INDEX idx_generation_jobs_requester_created_id ON generation_jobs (requested_by_user_id, created_at DESC, id DESC);
...
-- Capacity and export quotas
CREATE INDEX idx_quota_reservations_user_status ON quota_reservations (user_id, status);
CREATE INDEX idx_quota_reservations_user_period_status ON quota_reservations (user_id, period_key, status);
CREATE INDEX idx_quota_reservations_active_user ON quota_reservations (user_id, id) WHERE status = 'RESERVED';
CREATE INDEX idx_quota_reservations_active_export
    ON quota_reservations (user_id, period_key, quota_kind) WHERE status = 'RESERVED';
...
CREATE INDEX idx_media_assets_account_created_visible ON media_assets (account_id, created_at DESC, id DESC)
...
CREATE INDEX idx_narration_requests_project_voice_reference ON narration_requests (project_voice_reference_asset_id)
    WHERE project_voice_reference_asset_id IS NOT NULL;
CREATE INDEX idx_narration_requests_account_voice_reference ON narration_requests (account_voice_reference_asset_id)
...
CREATE INDEX idx_media_upload_sessions_account_status ON media_upload_sessions (account_id, status, created_at DESC);
...
CREATE INDEX idx_media_generation_items_review_queue ON media_generation_items (generation_job_id, review_status, item_key)
    WHERE review_status = 'NEEDS_REVIEW';
CREATE INDEX idx_media_generation_items_provider_operation ON media_generation_items (provider_operation_id)
    WHERE provider_operation_id IS NOT NULL;
...
CREATE INDEX idx_chapter_media_heads_updated ON chapter_media_heads (updated_at DESC, chapter_id);

-- Project render snapshots and desktop execution
CREATE INDEX idx_project_render_input_chapters_order ON project_render_input_chapters (generation_job_id, chapter_order_index);
CREATE INDEX idx_project_render_input_beats_order ON project_render_input_beats (generation_job_id, global_start_ms, scene_index, beat_index);
CREATE UNIQUE INDEX uq_final_artifacts_project_render_job ON final_artifacts (generation_job_id)
    WHERE artifact_type = 'PROJECT_VIDEO' AND generation_job_id IS NOT NULL AND status <> 'ARCHIVED';
CREATE INDEX idx_final_artifacts_project_video_fingerprint ON final_artifacts (project_id, render_fingerprint)
