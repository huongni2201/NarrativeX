...
from narrativex_gpu_worker.application import CapacityError, ExecutionApplicationService
from narrativex_gpu_worker.contracts import ComputeTask, ExecutionState
...
async def test_runtime_recovers_nonterminal_attempt_after_restart(
    ...
    first = ExecutionApplicationService(
        SqliteExecutionJournalAdapter(database_file), ExecutorRegistry((FakeExecutor(gate),)), 1
    )
    await first.start()
    ...
    second = ExecutionApplicationService(
        SqliteExecutionJournalAdapter(database_file), ExecutorRegistry((executor,)), 1
    )
    await second.start()
    try:
