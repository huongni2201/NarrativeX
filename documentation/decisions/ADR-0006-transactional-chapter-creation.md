# ADR-0018: Transactional chapter creation owns StoryVersion orchestration
...
The endpoint requires `Idempotency-Key`. PostgreSQL stores the owner/project/key and request fingerprint before the workflow and records the resulting Chapter on completion. Reusing the key returns the same Chapter; reusing it for a different request is a conflict.
