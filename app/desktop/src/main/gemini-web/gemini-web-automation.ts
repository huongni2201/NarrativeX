...
export class GeminiWebAutomation {
  ...
  private async triggerDownload(cdp: CdpClient, baseline: GenerationSnapshot): Promise<void> {
    const clickDownload = () =>
      evaluate<boolean>(
        ...
        `(() => {
          ...
          const candidates = [...document.querySelectorAll('button, a, [role="button"], [aria-label], [title]')].filter((element) => {
            if (!visible(element)) return false;
            const value = [element.getAttribute("aria-label"), element.getAttribute("title"), element.textContent]
              .filter(Boolean).join(" ").trim().toLowerCase();
            return words.some((word) => value.includes(word));
    ...
    throw geminiError(
      ...
      "The generated image is visible, but NarrativeX could not find Gemini's Download full size control after network capture fallback.",
    );
  }
