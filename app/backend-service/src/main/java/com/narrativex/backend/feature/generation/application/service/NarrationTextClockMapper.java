...
public final class NarrationTextClockMapper {
  ...
  private static Long mapStartOffset(int offset, List<WordAlignment> words) {
    ...
    for (WordAlignment word : words) {
      if (offset <= word.textStart()) {
        return word.audioStartMs();
      }
      if (offset <= word.textEnd()) {
        long textWidth = word.textEnd() - word.textStart();
        long audioWidth = word.audioEndMs() - word.audioStartMs();
        long relative = offset - word.textStart();
        return word.audioStartMs() + Math.round((double) relative * audioWidth / textWidth);
      }
      previous = word;
    }
