...
public class CharacterMyBatisRowMapper {
  ...
  public Character toDomain(CharacterRow row) {
    return Character.rehydrate(
        row.getId(), row.getRowVersion(), row.getOwnerId(), row.getWorkspaceId(),
        row.getCanonicalName(), read(row.getAliasesJson(), STRINGS),
  ...
  public CharacterVersion toDomain(CharacterVersionRow row) {
    return CharacterVersion.rehydrate(
        row.getId(), row.getRowVersion(), row.getCharacterId(), row.getVersionNumber(),
        row.getBible(), row.getVisualPrompt(), CharacterVersionStatus.valueOf(row.getStatus()),
        row.getLockedAt(), row.getLockedBy());
  }

  public OutfitVersion toDomain(OutfitVersionRow row) {
    return OutfitVersion.rehydrate(
        row.getId(), row.getRowVersion(), row.getCharacterId(), row.getVersionNumber(),
        row.getName(), row.getDescription(), row.getPrompt(),
  ...
  public CharacterAppearance toDomain(CharacterAppearanceRow row) {
    return CharacterAppearance.rehydrate(
        row.getId(), row.getRowVersion(), row.getCharacterId(), row.getProjectId(),
        row.getTimelineKey(), row.getAgeState(), row.getHairstyle(), row.getInjury(),
        row.getWardrobeContext(), row.getAppearancePrompt(), row.getOutfitVersionId());
  }

  public ProjectCharacter toDomain(ProjectCharacterRow row) {
    return ProjectCharacter.rehydrate(
        row.getId(), row.getRowVersion(), row.getProjectId(), row.getCharacterId(), row.getRole(),
        row.getImportance(), read(row.getProjectAliasesJson(), STRINGS), row.getStoryMetadata(),
        read(row.getGroupsJson(), STRINGS), row.getPinnedCharacterVersionId(),
  ...
  public CharacterRow row(Character value, InstantPair timestamps) {
    ...
    row.setId(value.getId()); row.setRowVersion(value.getRowVersion());
    row.setCreatedAt(timestamps.createdAt()); row.setUpdatedAt(timestamps.updatedAt());
    row.setOwnerId(value.getOwnerId()); row.setWorkspaceId(value.getWorkspaceId());
    row.setCanonicalName(value.getCanonicalName()); row.setAliasesJson(write(value.getAliases()));
  ...
  public CharacterVersionRow row(CharacterVersion value, InstantPair timestamps) {
    ...
    row.setId(value.getId()); row.setRowVersion(value.getRowVersion());
    row.setCreatedAt(timestamps.createdAt()); row.setUpdatedAt(timestamps.updatedAt());
    row.setCharacterId(value.getCharacterId()); row.setVersionNumber(value.getVersionNumber());
    row.setBible(value.getBible()); row.setVisualPrompt(value.getVisualPrompt());
    row.setStatus(value.getStatus().name()); row.setLockedAt(value.getLockedAt());
  ...
  public OutfitVersionRow row(OutfitVersion value, InstantPair timestamps) {
    ...
    row.setCreatedAt(timestamps.createdAt()); row.setUpdatedAt(timestamps.updatedAt());
    row.setCharacterId(value.getCharacterId()); row.setVersionNumber(value.getVersionNumber());
    row.setName(value.getName()); row.setDescription(value.getDescription());
    row.setPrompt(value.getPrompt()); row.setStatus(value.getStatus().name());
    return row;
  }
  ...
  public CharacterAppearanceRow row(CharacterAppearance value, InstantPair timestamps) {
    ...
    row.setId(value.getId()); row.setRowVersion(value.getRowVersion());
    row.setCreatedAt(timestamps.createdAt()); row.setUpdatedAt(timestamps.updatedAt());
    row.setCharacterId(value.getCharacterId()); row.setProjectId(value.getProjectId());
    row.setTimelineKey(value.getTimelineKey()); row.setAgeState(value.getAgeState());
    row.setHairstyle(value.getHairstyle()); row.setInjury(value.getInjury());
    row.setWardrobeContext(value.getWardrobeContext()); row.setAppearancePrompt(value.getAppearancePrompt());
  ...
  public ProjectCharacterRow row(ProjectCharacter value, InstantPair timestamps) {
    ...
    row.setId(value.getId()); row.setRowVersion(value.getRowVersion());
    row.setCreatedAt(timestamps.createdAt()); row.setUpdatedAt(timestamps.updatedAt());
    row.setProjectId(value.getProjectId()); row.setCharacterId(value.getCharacterId());
    row.setRole(value.getRole()); row.setImportance(value.getImportance());
    row.setProjectAliasesJson(write(value.getProjectAliases())); row.setStoryMetadata(value.getStoryMetadata());
    row.setGroupsJson(write(value.getGroups())); row.setPinnedCharacterVersionId(value.getPinnedCharacterVersionId());
    row.setStatus(value.getStatus().name());
    return row;
  }

  private <T> T read(String json, TypeReference<T> type) {
    try { return jsonMapper.readValue(json == null || json.isBlank() ? "[]" : json, type); }
    catch (Exception e) { throw new IllegalStateException("Invalid character JSON", e); }
  }

  private String write(Object value) {
    try { return jsonMapper.writeValueAsString(value); }
    catch (Exception e) { throw new IllegalStateException("Could not serialize character JSON", e); }
  }

  public record InstantPair(java.time.Instant createdAt, java.time.Instant updatedAt) {
    public static InstantPair now() { var now = java.time.Instant.now(); return new InstantPair(now, now); }
  }
}
