...
def test_visual_direction_v3_types_exist() -> None:
    required = {
        "ShotSize",
        "CameraAngle",
        "LensMm",
    ...
    assert not missing, f"missing Visual Direction V3 types: {missing}"
...
def test_visual_direction_accepts_independent_shot_size_and_camera_angle() -> None:
    direction_model = getattr(schema, "VisualDirectionV3", None)
    assert direction_model is not None, "VisualDirectionV3 must exist"

    direction = direction_model.model_validate(_direction())
...
def test_visual_direction_rejects_unsupported_lens() -> None:
    ...
    with pytest.raises(ValidationError):
        direction_model.model_validate(_direction(lens_mm=70))
...
def test_pan_requires_direction_but_push_in_does_not() -> None:
    ...
    with pytest.raises(ValidationError):
        direction_model.model_validate(
            _direction(camera_movement="PAN", movement_direction=None)
        )

    direction = direction_model.model_validate(
        _direction(camera_movement="PUSH_IN", movement_direction=None)
    )
