...
5. If a complete persisted beat audio clock already exists, preserve it. Otherwise derive semantic transition times on read from beat text starts and narration spans.
...
This avoids worker-to-worker reconciliation and is naturally independent of whether storyboard analysis or narration finishes first.
...
- [x] Python pure alignment regression: cumulative PCM timing and encoded duration normalization.
- [x] Visual source-anchor resolver tests include repeated text, Unicode/non-BMP UTF-16 offsets, out-of-order failure, and contiguous audio mapping.
- [x] Desktop subtitle/render timing regression passed in CI on the preceding code-equivalent head.
- [x] Repository gates passed in CI on the preceding code-equivalent head.
- [ ] Latest-head AI worker tests, Ruff, and Mypy all pass.
- [ ] Latest-head Backend verify passes Java 25 compile, unit/integration tests, formatting, and coverage gates.
- [ ] Latest-head Desktop check and Repository gates pass.
