...
export const BatchReviewToolbar: React.FC<BatchReviewToolbarProps> = ({ selectedCount, totalCount, onApprove, onRequestChanges, onReject, onClear }) => {
  ...
  return (
    <aside aria-label="Thao tác duyệt hàng loạt" className="fixed bottom-6 left-1/2 z-40 flex max-w-[90vw] -translate-x-1/2 items-center gap-6 rounded-2xl border border-primary/50 bg-surface/95 px-6 py-3 shadow-2xl backdrop-blur-xl motion-safe:animate-in motion-safe:slide-in-from-bottom-4 motion-safe:duration-200">
      <div className="flex items-center gap-3 border-r border-border-dark pr-4">
        <div className="flex h-6 w-6 items-center justify-center rounded-full border border-primary/60 bg-primary-muted text-xs font-bold text-primary-light">{selectedCount}</div>
